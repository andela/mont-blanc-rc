...
//*   attach productCategory schema to the collection     *//
ProductCategory.attachSchema(Schemas.ProductCategory);

if (ProductCategory.find().fetch().length.toString() === "0") {
  //* Insert product category object *//
  productCategoryList.forEach((productObject) => {
    ProductCategory.insert(productObject);
  });
}
/**
